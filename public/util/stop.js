...
const { auto_all, auto_bucket } = require( "../../components/start/transfer/auto" ),
  manualTransfer = require( "../../components/start/transfer/manual" );

let cb = () => console.log( col.black.bold.bgGreen( "Transfer Operation turned off." ) );
module.exports = async ( parent, str ) => {
  ...
  if ( all || bucket || Manual ) {
    if ( Manual ) await manualTransfer().stop().then( cb );
    if ( all ) await auto_all.stop().then( cb );
    if ( bucket ) await auto_bucket().stop().then( cb );
  } else {
