...
class StartComponent extends BaseUI {
    ...
    start(str,parent){
        ...
        if( container[1] === '--help' ||container.length===1 ) return this.startHelp()
        ...
        if(container[1] === 'transfer' && container.length===2) return this.master() 
        container.shift()
    ...
    autoBucket(){
        ...
        let callback = ({intervalTime,redisBuckets})=> this.auto_bucket.start(redisBuckets,intervalTime)
    ...
    manual(){
        ...
        let callback = ({redisBuckets})=>this.manualTransfer.start(redisBuckets)
        return ask.prompt(question,callback)   
    }
    master(){
